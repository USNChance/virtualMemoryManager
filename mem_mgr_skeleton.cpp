...
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) { 
    ...
    tlb[tlb_track].npage = page;
    tlb[tlb_track++].is_present = true;
...
void fifo_replace_page(size_t& frame ) { 
    int find = find_frame_ptable(frame);

    pg_table[find].is_present = false;
    pg_table[find].is_used = false;
}   

void lru_replace_page(size_t& frame) {
    int find = get_used_ptable();

    pg_table[find].is_present = true;
    frame = pg_table[find].frame_num;
 } 
